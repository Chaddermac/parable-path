...
import { PendingResponseSync } from "@/components/PendingResponseSync";
...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang="en" className={`${popularBody.variable} ${popularDisplay.variable}`}><body className={`experience-${experience}`}><PendingResponseSync /><SiteHeader experience={experience} />{children}<footer className={`shell mt-16 flex min-h-20 items-center justify-between border-t text-[10px] uppercase tracking-[.15em] ${experience === "popular" ? "popular-footer" : "border-ink/10 text-ink/50"}`}><span>House of Stories</span><span>{experience === "popular" ? "Find · Name · Share" : "Reflection, not a label"}</span></footer></body></html>;
}
